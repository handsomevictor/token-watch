...
const els = {
  ...
  ratePane: document.getElementById('ratePane'),
  rateChart: document.getElementById('rateChart'),
  rateRangeSelect: document.getElementById('rateRangeSelect'),
  rateEmpty: document.getElementById('rateEmpty'),
  rateMetricBtns: Array.from(document.querySelectorAll('[data-control="rateMetric"] .seg-btn')),
  rateGranBtns: Array.from(document.querySelectorAll('[data-control="rateGran"] .seg-btn'))
};

const RANGES = ['7', '30', '90', '365', 'all'];
const RATE_WINDOWS = ['1h', '3h', '6h', '12h', '1d', '1w'];
const state = { tab: 'activity', range: '30', stackBy: 'client', mode: 'bars', flat: false, locale: 'en', currency: 'USD', history: null, chartModel: null, chartKind: 'bars', rateMetric: 'total', rateWindow: '1h', rateGran: 0, rateModel: null, rateData: null };
...
function populateRateRangeSelect() {
  if (!els.rateRangeSelect) return;
  els.rateRangeSelect.innerHTML = RATE_WINDOWS.map((r) => `<button class="range-btn${r === state.rateWindow ? ' active' : ''}" data-val="${r}">${t(`dashboard.rate.window.${r}`)}</button>`).join('');
  els.rateRangeSelect.querySelectorAll('.range-btn').forEach((btn) => {
    btn.addEventListener('click', (e) => {
      state.rateWindow = e.target.dataset.val;
      els.rateRangeSelect.querySelectorAll('.range-btn').forEach((b) => b.classList.toggle('active', b === e.target));
      renderRate();
    });
  });
}
...
function formatRateTick(v) {
  return state.rateMetric === 'cost' ? formatCost(v) : `${formatCompact(v)}`;
}

function rateClockLabel(ms) {
  const d = new Date(ms);
  const hh = String(d.getHours()).padStart(2, '0');
  const mm = String(d.getMinutes()).padStart(2, '0');
  if (state.rateWindow === '1d' || state.rateWindow === '1w') return `${d.getMonth() + 1}/${d.getDate()} ${hh}:00`;
  return `${hh}:${mm}`;
}

async function renderRate() {
  if (!els.rateChart) return;
  els.rateMetricBtns.forEach((b) => b.classList.toggle('active', b.dataset.metric === state.rateMetric));
  els.rateGranBtns.forEach((b) => b.classList.toggle('active', Number(b.dataset.gran) === state.rateGran));
  const win = state.rateWindow;
  const metric = state.rateMetric;
  const gran = state.rateGran;
  let res = null;
  try { res = await window.tokenMonitor.getUsageRate(win, metric, gran); }
  catch (error) { console.log(`[dashboard] rate failed: ${error.message}`); }
  // Ignore a stale async result if the user switched window/metric/granularity meanwhile.
  ...
  const model = charts.lineAreaChart(points, { width: w, height: h, padTop: 12, padRight: 12, padBottom: 22, padLeft: 52 });
  state.rateModel = model;
  state.rateData = points;
  const n = points.length;
  const every = Math.max(1, Math.ceil(n / 6));
  els.rateChart.innerHTML = charts.lineAreaChartSvg(model, {
    yTicks: 4,
    formatTick: formatRateTick,
    axisLabel: (pt, i) => (i % every === 0 ? rateClockLabel(pt.t) : ''),
    titleOf: (pt) => `${rateClockLabel(pt.t)} · ${state.rateMetric === 'cost' ? formatCost(pt.value) + '/min' : formatCompact(pt.value) + (state.rateMetric === 'output' ? ' out/min' : ' tok/min')}`
  });
}
...
function render() {
  ...
  if (els.ratePane) els.ratePane.classList.toggle('hidden', state.tab !== 'rate');
  // The "no history" empty state only applies to the history-backed panes.
  if (state.tab === 'rate') els.empty.classList.add('hidden');
...
els.rateMetricBtns.forEach((b) => b.addEventListener('click', () => { state.rateMetric = b.dataset.metric; els.rateMetricBtns.forEach((x) => x.classList.toggle('active', x === b)); renderRate(); }));
els.rateGranBtns.forEach((b) => b.addEventListener('click', () => { state.rateGran = Number(b.dataset.gran); els.rateGranBtns.forEach((x) => x.classList.toggle('active', x === b)); renderRate(); }));
