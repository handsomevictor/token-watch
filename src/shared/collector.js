...
// Claude Code honors CLAUDE_CONFIG_DIR to relocate its data dir (e.g. ~/.claude-personal).
// tokscale hardcodes <home>/.claude/projects and ignores CLAUDE_CONFIG_DIR, so when that env
// var points elsewhere we build a synthetic "scan home" that mirrors the real home via symlinks
// but redirects .claude at the active config dir, and pass it to tokscale via --home. This keeps
// every other client (codex, cursor, …) resolving to their real locations.
// Resolved Claude data dir: an explicit user setting wins, else the CLAUDE_CONFIG_DIR
// env var, else the default ~/.claude. Set per collector run via setClaudeConfigDir().
let claudeConfigDirOverride = '';
function resolvedClaudeConfigDir() {
  return String(claudeConfigDirOverride || process.env.CLAUDE_CONFIG_DIR || '').trim();
}
function setClaudeConfigDir(dir) {
  const next = String(dir || '').trim();
  if (next !== claudeConfigDirOverride) {
    claudeConfigDirOverride = next;
    scanHomeComputed = false; // invalidate the cached scan-home symlink tree
    cachedScanHome = null;
  }
}

function claudeProjectsDir(home = os.homedir()) {
  const cfg = resolvedClaudeConfigDir();
  return cfg ? path.join(cfg, 'projects') : path.join(home, '.claude', 'projects');
}

let scanHomeComputed = false;
let cachedScanHome = null;
function tokscaleScanHome(realHome = os.homedir()) {
  if (scanHomeComputed) return cachedScanHome;
  scanHomeComputed = true;
  const cfg = resolvedClaudeConfigDir();
  if (!cfg || path.resolve(cfg) === path.join(realHome, '.claude')) return (cachedScanHome = null);
  try {
    const scanHome = path.join(sharedDataDir(), 'scan-home');
    fs.mkdirSync(scanHome, { recursive: true });
    for (const entry of fs.readdirSync(realHome)) {
      if (entry === '.claude') continue;
      const link = path.join(scanHome, entry);
      if (!fs.existsSync(link)) {
        try { fs.symlinkSync(path.join(realHome, entry), link); } catch (_) {}
      }
    }
    const claudeLink = path.join(scanHome, '.claude');
    try { fs.rmSync(claudeLink, { force: true }); } catch (_) {}
    fs.symlinkSync(path.resolve(cfg), claudeLink);
    return (cachedScanHome = scanHome);
  } catch (_) {
    return (cachedScanHome = null);
  }
}

function tokscaleHomeArgs() {
  const scanHome = tokscaleScanHome();
  return scanHome ? ['--home', scanHome] : [];
}

function spawnTokscaleJson(userArgs, commandTimeoutMs) {
  const { bin, prefixArgs, env } = tokscaleCommand();
  return new Promise((resolve, reject) => {
    const child = spawn(bin, [...prefixArgs, ...tokscaleHomeArgs(), ...userArgs], { env, windowsHide: true });
    let stdout = '';
    let stderr = '';
...
// Per-hour usage buckets for the coarse (1d/1w) rate-graph windows. `since` is YYYY-MM-DD.
function runTokscaleHourly({ clients, since, commandTimeoutMs }) {
  const args = ['hourly', '--json', '--no-spinner'];
  if (clients) args.push('--client', clients);
  if (since) args.push('--since', since);
  return spawnTokscaleJson(args, commandTimeoutMs);
}
...
function readFileHead(filePath, bytes = 16 * 1024) {
  let fd;
  try {
    fd = fs.openSync(filePath, 'r');
    const stat = fs.fstatSync(fd);
...
// Human-readable identifiers for a session, from its transcript:
//  - project: the final path segment of `cwd` (e.g. "token-watch"), found in message
//    lines (tail).
//  - title: Claude Code's AI-generated session title (`aiTitle`), written near the
//    start (head) — the best way to tell same-project sessions apart.
function sessionMetaFromFile(filePath) {
  let project = '';
  let title = '';
  const scan = (text) => {
    if (!text) return;
    if (!project) {
      ...
      if (m && m[1]) project = m[1].replace(/[\\/]+$/, '').split(/[\\/]/).filter(Boolean).pop() || '';
    }
    if (!title) {
      const m = text.match(/"aiTitle"\s*:\s*"((?:[^"\\]|\\.)*)"/);
      if (m && m[1]) { try { title = JSON.parse(`"${m[1]}"`); } catch (_) { title = m[1]; } }
    }
  };
  // cwd and aiTitle both recur through the transcript, so the tail usually has both;
  // fall back to the head for short/edge-case files.
  try { scan(readFileTail(filePath)); } catch (_) {}
  if (!project || !title) { try { scan(readFileHead(filePath)); } catch (_) {} }
...
function sessionTimestampMap(periods, home = os.homedir(), deps = {}) {
  ...
  const applyFile = (client, sessionId, filePath) => {
    ...
    const { project, title } = sessionMetaFromFile(filePath);
    metadata.set(`${client}:${sessionId}`, { startedAt, lastUsedAt, project, title });
  };
  ...
  const claudeFiles = findSessionFiles(claudeProjectsDir(home), byClient.get('claude') || []);
...
function applySessionTimestamps(periods, home, deps = {}) {
  ...
  for (const period of Object.values(periods || {})) {
    for (const [key, session] of Object.entries(period?.sessions || {})) {
      ...
      if (meta.project && !session.project) session.project = meta.project;
      if (meta.title && !session.title) session.title = meta.title;
    }
  }
}
...
async function maybeSyncAntigravity(clientsCsv, logger, home = os.homedir()) {
  ...
  await new Promise((resolve) => {
    const child = spawn(bin, [...prefixArgs, ...tokscaleHomeArgs(), 'antigravity', 'sync'], { env, windowsHide: true });
    let stderr = '';
...
async function collectUsageOnce(options) {
  ...
  setClaudeConfigDir(options.claudeConfigDir);
...
function clientWatchCandidates(clientsCsv) {
  ...
  const claudeConfigDir = resolvedClaudeConfigDir() || path.join(home, '.claude');
  add('claude', claudeProjectsDir(home), path.join(claudeConfigDir, 'transcripts'));
