...
function readSessionDetail({ client, sessionId, period = 'total', sessionCost = 0, home, claudeConfigDir = '', deps = {} }) {
  if (client === 'opencode') return readOpenCodeSessionDetail({ sessionId, period, deps });
  const filePath = resolveSessionFile(client, sessionId, home, claudeConfigDir);
