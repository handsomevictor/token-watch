  ...
  function sessionRowsForPeriod(period, options = {}) {
    ...
    const rows = Object.entries(period?.sessions || {})
      .map(([key, session]) => {
        ...
        // Lead with the project folder (from the session's cwd) so rows are
        // identifiable at a glance; fall back to the client label when unknown.
        const titleParts = [project || clientLabel, modelLabel].filter(Boolean);
        const sessionId = session?.sessionId || key;
        const subtitleParts = [
          sessionActivityLabel(session, now),
          messageLabel(session)
        ].filter(Boolean);
        const aiTitle = String(session?.title || '').trim();
        return {
          key: `session:${key}`,
          kind: 'session',
          name: titleParts.join(' · '),
          subtitle: subtitleParts.join(' · '),
          // Prefer the AI-generated session title over the opaque UUID so that
          // multiple sessions in the same project are still distinguishable.
          detail: aiTitle || sessionIdLabel(sessionId),
          value,
