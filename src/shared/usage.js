...
function mergeSession(target, source) {
  if (!target.project && source.project) target.project = String(source.project);
  if (!target.title && source.title) target.title = String(source.title);
...
function normalizeSession(input, fallbackKey) {
  ...
  if (input.project) session.project = String(input.project);
