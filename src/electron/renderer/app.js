...
Object.assign(els, {
  ...
  usageRefreshInput: document.getElementById('usageRefreshInput'),
  numberAnimationInput: document.getElementById('numberAnimationInput'),
  alwaysOnTopAboveAllInput: document.getElementById('alwaysOnTopAboveAllInput'),
  modelSortByCostInput: document.getElementById('modelSortByCostInput'),
  claudeDataDirInput: document.getElementById('claudeDataDirInput'),
  dailyBudgetInput: document.getElementById('dailyBudgetInput'),
  monthlyBudgetInput: document.getElementById('monthlyBudgetInput'),
  customPricingList: document.getElementById('customPricingList'),
...
function metaDayKey(offsetDays = 0) {
  const d = new Date();
  d.setDate(d.getDate() - offsetDays);
  return `${d.getFullYear()}-${String(d.getMonth() + 1).padStart(2, '0')}-${String(d.getDate()).padStart(2, '0')}`;
}

const budgetNotified = {};
function activeBudget() {
  if (state.period === 'today') return Number(state.settings?.dailyBudget || 0);
  if (state.period === 'month') return Number(state.settings?.monthlyBudget || 0);
  return 0;
}
function maybeNotifyBudget(cost, budget) {
  if (!(budget > 0) || !window.tokenMonitor?.showNotification) return;
  const pct = cost / budget;
  const bucket = pct >= 1 ? 100 : pct >= 0.8 ? 80 : pct >= 0.5 ? 50 : 0;
  const stamp = state.period === 'today' ? metaDayKey(0) : metaDayKey(0).slice(0, 7);
  const key = `${state.period}:${stamp}`;
  const prev = budgetNotified[key] || 0;
  if (bucket > prev) {
    budgetNotified[key] = bucket;
    if (bucket >= 50) {
      const label = state.period === 'today' ? t('costMeta.budgetDay') : t('costMeta.budgetMonth');
      window.tokenMonitor.showNotification(
        t('costMeta.budgetTitle', { pct: bucket, label }),
        t('costMeta.budgetBody', { cost: formatCost(cost), budget: formatCost(budget) })
      );
    }
  }
}
function renderCostMeta(period) {
  const el = els.costMeta;
  if (!el) return;
  const cost = Number(period.costUsd || 0);
  const parts = [];
  if (els.cost) els.cost.classList.remove('near-budget', 'over-budget');
  const budget = activeBudget();
  if (budget > 0) {
    if (els.cost) els.cost.classList.add(cost >= budget ? 'over-budget' : (cost >= budget * 0.8 ? 'near-budget' : 'within-budget'));
    if (els.cost) els.cost.classList.remove('within-budget');
    parts.push(t('costMeta.budget', { pct: Math.round((cost / budget) * 100), budget: formatCost(budget) }));
    maybeNotifyBudget(cost, budget);
  }
  if (state.period === 'today') {
    const daily = state.stats?.historyPreview?.daily || [];
    const yKey = metaDayKey(1);
    const yday = daily.find((d) => String(d.date).slice(0, 10) === yKey);
    const yCost = Number(yday?.cost || 0);
    if (yCost > 0) {
      const pct = Math.round(((cost - yCost) / yCost) * 100);
      parts.push(t('costMeta.vsYesterday', { sign: pct >= 0 ? '↑' : '↓', pct: Math.abs(pct) }));
    }
  } else if (state.period === 'month') {
    const now = new Date();
    const day = now.getDate();
    const daysInMonth = new Date(now.getFullYear(), now.getMonth() + 1, 0).getDate();
    if (day > 0 && cost > 0) parts.push(t('costMeta.projected', { amount: formatCost(cost / day * daysInMonth) }));
  }
...
function animateNumber(el, from, to, duration = 600) {
  ...
  if (state.settings && state.settings.numberAnimation === false) { el.textContent = formatNumber(to); return; }
...
function updateRow(row, { name, subtitle, detail, value, cost, max, color, stale, platform, local, client, kind, title, cacheReadTokens, cacheWriteTokens, outputTokens, costPrimary }) {
  const width = rowWidth(costPrimary ? Number(cost || 0) : value, max);
  const isExpanded = row.classList.contains('expanded');
  ...
  if (costPrimary) {
    row.querySelector('.row-value').textContent = formatCost(cost || 0);
    row.querySelector('.row-cost').textContent = formatNumber(value);
  } else {
...
function renderRows(rows) {
  ...
  const costPrimary = rows.length > 0 && rows[0].costPrimary === true;
  const max = Math.max(costPrimary ? 0.0001 : 1, ...rows.map((row) => costPrimary ? Number(row.cost || 0) : row.value));
  const signature = `${state.breakdown}\n${costPrimary ? 'cost' : 'tok'}\n${rows.map((row) => row.key).join('\n')}`;
...
function modelRowsForPeriod(period) {
  ...
  if (modelRows.length > 0) {
    if (state.settings && state.settings.modelSortByCost) {
      for (const row of modelRows) row.costPrimary = true;
      return modelRows.sort((a, b) => b.cost - a.cost || b.value - a.value);
    }
...
const VIEW_LABEL_BY_ID = new Map(VIEW_DISPLAY_OPTIONS.map((o) => [o.id, o.labelKey]));
// Top horizontal tab bar: one clickable tab per enabled view (honors hidden/order
// preferences), highlighting the active one. Replaces the old footer cycle button.
function renderBreakdownTabs() {
  if (!els.viewTabs) return;
  const order = visibleBreakdownOrder();
  els.viewTabs.classList.toggle('hidden', order.length <= 1);
  const sig = `${order.join(',')}|${state.breakdown}`;
  if (els.viewTabs.dataset.sig !== sig) {
    els.viewTabs.replaceChildren(...order.map((id) => {
      const btn = document.createElement('button');
      btn.type = 'button';
      btn.className = `view-tab${id === state.breakdown ? ' active' : ''}`;
      btn.dataset.view = id;
      btn.textContent = t(VIEW_LABEL_BY_ID.get(id) || id) || id;
      return btn;
    }));
    els.viewTabs.dataset.sig = sig;
  } else {
    for (const btn of els.viewTabs.children) btn.classList.toggle('active', btn.dataset.view === state.breakdown);
  }
}
...
function applyControlLayout(settingsInTitlebar) {
  ...
  // When the gear lives in the titlebar, keep the actions row always visible so the
  // user never has to discover the hover hotspot to reach Settings.
  if (els.shell) els.shell.classList.toggle('settings-in-titlebar', settingsInTitlebar === true);
}
...
function syncSettingsForm() {
  ...
  if (els.usageRefreshInput) els.usageRefreshInput.value = String([15000, 30000, 60000, 120000, 300000, 600000, 900000].includes(Number(state.settings.usageRefreshMs)) ? state.settings.usageRefreshMs : 300000);
  if (els.numberAnimationInput) els.numberAnimationInput.checked = state.settings.numberAnimation !== false;
  if (els.alwaysOnTopAboveAllInput) els.alwaysOnTopAboveAllInput.checked = Boolean(state.settings.alwaysOnTopAboveAll);
  if (els.modelSortByCostInput) els.modelSortByCostInput.checked = Boolean(state.settings.modelSortByCost);
  void populateClaudeDataDirOptions();
  void renderCustomPricing();
  if (els.dailyBudgetInput) els.dailyBudgetInput.value = Number(state.settings.dailyBudget) > 0 ? String(state.settings.dailyBudget) : '';
  if (els.monthlyBudgetInput) els.monthlyBudgetInput.value = Number(state.settings.monthlyBudget) > 0 ? String(state.settings.monthlyBudget) : '';
...
async function populateClaudeDataDirOptions() {
  const select = els.claudeDataDirInput;
  if (!select || !window.tokenMonitor?.dataDir) return;
  let candidates = [];
  try { candidates = (await window.tokenMonitor.dataDir.listCandidates()) || []; } catch (_) {}
  const current = (state.settings?.dataDirs && state.settings.dataDirs.claude) || '';
  const opts = [{ value: '', label: t('settings.tools.dataDirAuto') }];
  const seen = new Set(['']);
  for (const c of candidates) {
    if (!c || seen.has(c.path)) continue;
    seen.add(c.path);
    opts.push({ value: c.path, label: c.source ? `${c.path} · ${c.source}` : c.path });
  }
  if (current && !seen.has(current)) opts.push({ value: current, label: current });
  opts.push({ value: '__browse__', label: t('settings.tools.dataDirBrowse') });
  select.replaceChildren(...opts.map((o) => {
...
const PRICING_EPS = 0.001;
function pricingEsc(s) { return String(s).replace(/[&<>"]/g, (c) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;' }[c])); }
async function renderCustomPricing() {
  const list = els.customPricingList;
  if (!list || !window.tokenMonitor?.pricing) return;
  let overrides = {};
  try { overrides = (await window.tokenMonitor.pricing.get())?.models || {}; } catch (_) {}
  const seen = new Set();
  for (const p of Object.values(state.stats?.periods || {})) for (const m of Object.keys(p?.models || {})) seen.add(m);
  for (const m of Object.keys(overrides)) seen.add(m);
  const models = [...seen].filter(Boolean).sort();
  if (!models.length) { list.innerHTML = `<p class="settings-note">${pricingEsc(t('settings.pricing.empty'))}</p>`; return; }
  list.replaceChildren(...models.map((id) => {
    const ov = overrides[id] || {};
    const isFree = Number(ov.input_cost_per_million_tokens) > 0 && Number(ov.input_cost_per_million_tokens) <= PRICING_EPS && Number(ov.output_cost_per_million_tokens) <= PRICING_EPS;
    const val = (f) => (ov[f] !== undefined && !isFree ? String(ov[f]) : '');
    const row = document.createElement('div');
    row.className = 'pricing-row';
    row.dataset.model = id;
    row.innerHTML = `<span class="pricing-model" title="${pricingEsc(id)}">${pricingEsc(id)}</span>`
      + `<label class="pricing-free"><input type="checkbox" class="pricing-free-input" ${isFree ? 'checked' : ''}/>${pricingEsc(t('settings.pricing.free'))}</label>`
      + `<input class="pricing-num pricing-in" type="number" min="0" step="0.01" placeholder="in" value="${val('input_cost_per_million_tokens')}"${isFree ? ' disabled' : ''}/>`
      + `<input class="pricing-num pricing-out" type="number" min="0" step="0.01" placeholder="out" value="${val('output_cost_per_million_tokens')}"${isFree ? ' disabled' : ''}/>`
      + `<input class="pricing-num pricing-cr" type="number" min="0" step="0.01" placeholder="cR" value="${val('cache_read_input_token_cost_per_million_tokens')}"${isFree ? ' disabled' : ''}/>`
      + `<input class="pricing-num pricing-cw" type="number" min="0" step="0.01" placeholder="cW" value="${val('cache_creation_input_token_cost_per_million_tokens')}"${isFree ? ' disabled' : ''}/>`;
    return row;
  }));
}
async function saveCustomPricing() {
  if (!els.customPricingList) return;
  const models = {};
  for (const row of els.customPricingList.querySelectorAll('.pricing-row')) {
    const id = row.dataset.model;
    if (!id) continue;
    if (row.querySelector('.pricing-free-input')?.checked) {
      models[id] = { input_cost_per_million_tokens: PRICING_EPS, output_cost_per_million_tokens: PRICING_EPS, cache_read_input_token_cost_per_million_tokens: PRICING_EPS, cache_creation_input_token_cost_per_million_tokens: PRICING_EPS };
      continue;
    }
    const num = (sel) => { const v = row.querySelector(sel)?.value; const n = Number(v); return v !== '' && Number.isFinite(n) && n >= 0 ? n : undefined; };
    const inp = num('.pricing-in'), out = num('.pricing-out'), cr = num('.pricing-cr'), cw = num('.pricing-cw');
    if (!(inp > 0 || out > 0)) continue; // no/invalid override → fall back to default price
    const e = {};
    if (inp !== undefined) e.input_cost_per_million_tokens = inp;
    if (out !== undefined) e.output_cost_per_million_tokens = out;
    if (cr !== undefined) e.cache_read_input_token_cost_per_million_tokens = cr;
    if (cw !== undefined) e.cache_creation_input_token_cost_per_million_tokens = cw;
    models[id] = e;
  }
  if (els.savePricingButton) { els.savePricingButton.disabled = true; els.savePricingButton.textContent = t('settings.pricing.saving'); }
  try { await window.tokenMonitor.pricing.set(models); } catch (_) {}
  await renderCustomPricing();
  if (els.savePricingButton) { els.savePricingButton.disabled = false; els.savePricingButton.textContent = t('settings.pricing.save'); }
}
...
els.usageRefreshInput?.addEventListener('change', () => saveSettings({ usageRefreshMs: Number(els.usageRefreshInput.value) }));
els.numberAnimationInput?.addEventListener('change', () => saveSettings({ numberAnimation: els.numberAnimationInput.checked }));
els.modelSortByCostInput?.addEventListener('change', async () => { await saveSettings({ modelSortByCost: els.modelSortByCostInput.checked }); render(); });
els.dailyBudgetInput?.addEventListener('change', async () => { await saveSettings({ dailyBudget: Math.max(0, Number(els.dailyBudgetInput.value) || 0) }); render(); });
els.monthlyBudgetInput?.addEventListener('change', async () => { await saveSettings({ monthlyBudget: Math.max(0, Number(els.monthlyBudgetInput.value) || 0) }); render(); });
els.savePricingButton?.addEventListener('click', () => void saveCustomPricing());
els.customPricingList?.addEventListener('change', (event) => {
  const cb = event.target.closest('.pricing-free-input');
  if (!cb) return;
  const row = cb.closest('.pricing-row');
  if (row) row.querySelectorAll('.pricing-num').forEach((inp) => { inp.disabled = cb.checked; });
});
els.claudeDataDirInput?.addEventListener('change', async () => {
  const value = els.claudeDataDirInput.value;
  const currentClaude = (state.settings?.dataDirs && state.settings.dataDirs.claude) || '';
  if (value === '__browse__') {
    let picked = null;
    try { picked = await window.tokenMonitor.dataDir.browse(); } catch (_) {}
    if (picked) await saveSettings({ dataDirs: { ...(state.settings.dataDirs || {}), claude: picked } });
    await populateClaudeDataDirOptions();
    return;
  }
  if (value !== currentClaude) await saveSettings({ dataDirs: { ...(state.settings.dataDirs || {}), claude: value } });
});
els.alwaysOnTopAboveAllInput?.addEventListener('change', () => {
  const on = els.alwaysOnTopAboveAllInput.checked;
  saveSettings(on ? { windowBehavior: 'floating', alwaysOnTopAboveAll: true } : { alwaysOnTopAboveAll: false });
});
