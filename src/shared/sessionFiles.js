...
function resolveSessionFile(client, sessionId, home, claudeConfigDir = '') {
  const id = String(sessionId || '');
  if (!id) return '';
  if (client === 'claude') {
    const root = claudeConfigDir ? path.join(claudeConfigDir, 'projects') : path.join(home, '.claude', 'projects');
    return findSessionFiles(root, [id]).get(id) || '';
  }
  if (client === 'codex') {
