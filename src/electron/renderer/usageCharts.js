...
(function exposeUsageCharts(root, factory) {
  ...
  // Time-series line/area chart for the usage-rate view. points: [{t (ms), value}].
  function lineAreaChart(points, options) {
    const o = Object.assign({ width: 600, height: 200, padTop: 10, padRight: 10, padBottom: 22, padLeft: 48 }, options || {});
    const pts = Array.isArray(points) ? points : [];
    const innerW = o.width - o.padLeft - o.padRight;
    const innerH = o.height - o.padTop - o.padBottom;
    const maxVal = Math.max(1e-9, ...pts.map((p) => Number(p.value) || 0));
    ...
    const mapped = pts.map((p, i) => ({
      ...
      x: o.padLeft + innerW * (p.t - tMin) / span,
      y: o.padTop + innerH - innerH * (Number(p.value) || 0) / maxVal
    }));
    return { width: o.width, height: o.height, plot: { x: o.padLeft, y: o.padTop, w: innerW, h: innerH }, maxVal, tMin, tMax, points: mapped };
  }

  function lineAreaChartSvg(model, options) {
    const o = Object.assign({ yTicks: 4, formatTick: (v) => String(v), titleOf: null, axisLabel: null }, options || {});
    const p = model.plot;
    const grid = yAxisSvg(p, model.maxVal, o.yTicks, o.formatTick);
    const pts = model.points || [];
    if (!pts.length) {
      return `<svg class="rate-chart" viewBox="0 0 ${model.width} ${model.height}" width="100%" height="${model.height}">${grid}</svg>`;
    }
    const line = pts.map((pt, i) => `${i === 0 ? 'M' : 'L'}${svgRound(pt.x)},${svgRound(pt.y)}`).join(' ');
    const baseY = p.y + p.h;
    const area = `M${svgRound(pts[0].x)},${svgRound(baseY)} ` + pts.map((pt) => `L${svgRound(pt.x)},${svgRound(pt.y)}`).join(' ') + ` L${svgRound(pts[pts.length - 1].x)},${svgRound(baseY)} Z`;
    const slot = pts.length > 1 ? (pts[1].x - pts[0].x) : p.w;
    const hovers = pts.map((pt) => {
      const tip = o.titleOf ? o.titleOf(pt) : '';
      const title = tip ? `<title>${escapeXml(tip)}</title>` : '';
      return `<rect data-i="${pt.i}" x="${svgRound(pt.x - slot / 2)}" y="${svgRound(p.y)}" width="${svgRound(slot)}" height="${svgRound(p.h)}" class="bar-hover">${title}</rect>`;
    }).join('');
    const axis = o.axisLabel ? pts.map((pt, i) => { const l = o.axisLabel(pt, i); return l ? axisText(l, pt.x, p.y + p.h + 14) : ''; }).join('') : '';
    const baseline = `<line class="axis-base" x1="${svgRound(p.x)}" y1="${svgRound(baseY)}" x2="${svgRound(p.x + p.w)}" y2="${svgRound(baseY)}"></line>`;
    return `<svg class="rate-chart" viewBox="0 0 ${model.width} ${model.height}" width="100%" height="${model.height}">${grid}<path class="rate-area" d="${area}"></path><path class="rate-line" d="${line}" fill="none"></path>${baseline}${axis}${hovers}</svg>`;
  }
  ...
  return {
    ...
    barsChartSvg, candleChartSvg, heatmapSvg, statsCardsHtml, lineAreaChart, lineAreaChartSvg
  };
});
